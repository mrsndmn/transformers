...
def plot_log_freq_vs_hopping_potential(token_potential_min_df, output_dir):
    ...
    plt.tight_layout()
    file_path = os.path.join(output_dir, "boxplot_log_freq_bins_vs_mean_hopping_potential.png")
    plt.savefig(file_path)
    print(f"Saved boxplot to {file_path}")
    plt.close()
    return file_path

def plot_hopping_potential_distribution(token_potential_min_df, output_dir):
    H_cols = [col for col in token_potential_min_df.columns if col.startswith("H")]
...
def plot_token_occurrences_distribution(token_occurences, output_dir):
    ...
    file_path = os.path.join(output_dir, "token_occurences_distribution.png")
    plt.savefig(file_path)
...
def main():
    ...
    ranked_h, mean_hopping_potential_corr = analyze_hopping_potentials(token_potential_min_df)
