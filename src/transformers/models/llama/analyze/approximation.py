...
if __name__ == "__main__":
    ...
    for token_id in most_common_tokens:
        ...
        plot_heatmap(overall_additive_cosine_sim, file_path, f"Overall Cosine Similarity. Additive Approximation. Token {token_id} ({token_str})", vmin=0, vmax=1)
        ...
        plot_heatmap(overall_noop_cosine_sim, file_path, f"Overall Cosine Similarity. Noop. Token {token_id} ({token_str})", vmin=0, vmax=1)
        ...
        plot_heatmap(overall_scale_cosine_sim, file_path, f"Overall Cosine Similarity. Scale Multiplicative Approximation. Token {token_id} ({token_str})", vmin=0, vmax=1)

        overall_additive_l1_dist = torch.stack(overall_additive_l1_dist).mean(dim=1)
        overall_additive_cosine_sim = torch.stack(overall_additive_cosine_sim).mean(dim=1)
        overall_noop_l1_dist = torch.stack(overall_noop_l1_dist).mean(dim=1)
        overall_noop_cosine_sim = torch.stack(overall_noop_cosine_sim).mean(dim=1)
        overall_scale_l1_dist = torch.stack(overall_scale_l1_dist).mean(dim=1)
        overall_scale_cosine_sim = torch.stack(overall_scale_cosine_sim).mean(dim=1)

        # Line plot cosine similarity
        plt.figure(figsize=(10, 6))
        plt.plot(overall_additive_cosine_sim, label='Additive Approximation')
        plt.plot(overall_noop_cosine_sim, label='Noop')
        plt.plot(overall_scale_cosine_sim, label='Scale Multiplicative Approximation')
        plt.xlabel('Layer')
        plt.ylabel('Cosine Similarity')
        plt.title(f'Cosine Similarity of Approximations for Token {token_id} ({token_str})')
        plt.legend()
        file_path = os.path.join(plots_dir, f"overall_cosine_similarity_layer_{token_id}.png")
        plt.savefig(file_path)
        print(f"saved {file_path}")
        plt.close()

        # Line plot L1 distance
        plt.figure(figsize=(10, 6))
        plt.plot(overall_additive_l1_dist, label='Additive Approximation')
        plt.plot(overall_noop_l1_dist, label='Noop')
        plt.plot(overall_scale_l1_dist, label='Scale Multiplicative Approximation')
        plt.xlabel('Layer')
        plt.ylabel('L1 Distance')
        plt.title(f'L1 Distance of Approximations for Token {token_id} ({token_str})')
        plt.legend()
        file_path = os.path.join(plots_dir, f"overall_l1_distance_layer_{token_id}.png")
        plt.savefig(file_path)
        print(f"saved {file_path}")
        plt.close()
