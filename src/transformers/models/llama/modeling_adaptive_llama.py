...
class AdaptiveLlamaModelWithEachLayerPruning(AdaptiveLlamaPreTrainedModel):
    def __init__(self, config: LlamaConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        ...
        print("Loading Each Layer Pruning adaptive llama")
    ...
    def forward(
        ...
        for layer_idx in range(self.config.num_hidden_layers):
            ...
            adaptive_down_output: AdaptiveFanInOutput = self.fan_in_layers[layer_idx](
                input_ids=input_ids,
                hidden_state=hidden_states,
                attention_mask=fan_in_attention_mask,
                special_embeddings_mask=fan_in_special_embeddings_mask,
            ...
            if not self.fan_in_layers[layer_idx].training:
                # Prefill
                if hidden_states is not None:
                    new_seq_len = hidden_states.shape[1]
                ...
                # Decode
                if past_key_values is not None and len(past_key_values.key_cache) > layer_idx+1:
                    if hidden_states is not None:
                        fan_in_cache_position = past_key_values.key_cache[layer_idx+1].shape[2]
                        loop_down_cache_position = torch.tensor([ fan_in_cache_position ], device=cache_position.device, dtype=torch.long)
                        loop_down_position_ids = loop_down_cache_position.unsqueeze(0)
...
class AdaptiveLlamaForCausalLMWithEachLayerPruning(AdaptiveLlamaPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
