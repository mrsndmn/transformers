    ...
    for pretrained_model_name in ['HuggingFaceTB/SmolLM2-1.7B', 'unsloth/Llama-3.2-1B', 'Qwen/Qwen3-1.7B']:
        ...
        for shard_i in range(num_shards):
            ...
            result = client.run_job(
                payload={
                    'script': f"bash -c 'cd {workdir} && /workspace-SR004.nfs2/d.tarasov/envs/tokens_pruning/bin/python src/transformers/models/llama/tmp/tokenize_fineweb_edu.py --shard_num {shard_i} --total_shards {num_shards} --pretrained_model_name {pretrained_model_name} --with_eos_token'",
