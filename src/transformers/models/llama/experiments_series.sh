...
# * Почему лосс не падает когда мерджим без доп лосса? Мб мешают резидуалы?
# * Кастомное cuda ядро для того, чтобы вычислять мерджинг мапу? Тк сейчас работает очень долго - по 10 секунд на одну итерацию, тк есть часть кода, которая не векторизуется
